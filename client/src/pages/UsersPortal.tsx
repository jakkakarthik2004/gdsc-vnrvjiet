...
function UserPortal() {
  ...
  return (
    <div className="min-h-full p-4">
      <div className="HeroSection flex flex-col  bg-cover bg-center bg-no-repeat m-4"  style={heroStyle}>
        <img src=""/>
        <h2 className="text-2xl font-bold mb-4 my-auto">Welcome back, [Username].</h2>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {events.map((event) => (
            <div
              key={event.eventId}
              className="bg-white p-4 rounded shadow-md shadow-lg max-w-sm hover:scale-105 duration-200 border border-slate-400 p-4"
            >
              <h4 className="text-lg font-semibold mb-3">{event.name}</h4>
              <p> <strong>What's Happening : </strong>{event.description}</p>
